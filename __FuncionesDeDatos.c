...
void MigrarDatos(char * NArchOrg, char * NArchDst){
	...
	FILE * ArchOrgn = fopen(NArchOrg,"r");
	FILE * ArchDstn = fopen(NArchDst,"r+b");
	...
	//Si alguno de los dos archivos es nulo, la función avisa de esto pero no hace ningún tipo de migración. CAL 27/10/2022
		...
		if (ArchDstn == NULL) printf("\nHay un problema con la apertura del archivo destino (de extensi%cn '.DAT'). Verifique que exista.\n",162);
	}
...
void BajaFisica(char * NArchDAT, const char * NombreArchBajasFis){
	...
	*/
	...
	FILE * ArchDat = fopen(NArchDAT,"rb+");
	FILE * ArchBajas = fopen(NombreArchBajasFis,"ab+");
